...
function run() {
  ...
  var svg = d3.select("#sortablebarchart1").append("svg")
  ...
  var e1 = document.getElementById("category7");
  var category = e.options[e.selectedIndex].value;
  var player = e1.options[e1.selectedIndex].value;

  d3.tsv("/data/buccaneers/rushing.txt", function(error, data) {

    data.forEach(function(d) {
      if (d.name==player) {
        if (category == "att") d.frequency =+ d.att;
...
function updateData1() {
  ...
  d3.tsv("/data/buccaneers/rushing.txt", function(error, data) {
    ...
    x.domain(data.map(function(d) { return d.week; }));
    y.domain([0, d3.max(data, function(d) { return d.frequency; })]);
    var svg = d3.select("#sortablebarchart1").transition();
    count = 0;
